...
/**
 * Defines currencies available to the CowPro currency formatting and price APIs.
 *
 * By default Drupal CowPro defines all actively traded currencies according
 ...
 *     Default: 1
 *
 * @see hook_cowpro_currency_info_alter()
 */
function hook_cowpro_currency_info() {
  return array (
    'CHF' => array(
      'code' => 'CHF',
      'numeric_code' => '756',
...
/**
 * Allows modules to alter CowPro currency definitions.
 *
 * By default CowPro provides all active currencies according to ISO 4217.
 ...
 *
 * @see hook_cowpro_currency_info()
 */
function hook_cowpro_currency_info_alter(&$currencies, $langcode) {
 ...
 */
function hook_cowpro_entity_access($op, $entity, $account, $entity_type) {
  // No example.
}

/**
 * Allows modules to alter the conditions used on the query to grant view access
 * to a CowPro entity of the specified ENTITY TYPE.
 *
 * The CowPro module defines a generic implementation of hook_query_alter() to
 * determine view access for its entities, cowpro_entity_access_query_alter().
 * This function is called by modules defining CowPro entities from their
 ...
 *
 * @see cowpro_entity_access_query_alter()
 * @see cowpro_cart_cowpro_entity_access_condition_cowpro_order_alter()
 */
function hook_cowpro_entity_access_condition_ENTITY_TYPE_alter() {
 ...
 * invoked after it.
 *
 * @see hook_cowpro_entity_access_condition_ENTITY_TYPE_alter()
 */
function hook_cowpro_entity_access_condition_alter() {
  // No example.
}

/**
 * Allows modules to alter newly created CowPro entities.
 *
 * CowPro's default entity controller, DrupalCowProEntityController, invokes
 ...
 */
function hook_cowpro_entity_create_alter($entity_type, $entity) {
  // No example.
}
